...
public static void main(String[] args) throws IOException {
		...
		int taille = ErrorRate.will.getn();

		// Recuperer tailles des clusters

		int[] TaillesCluster = ErrorRate.will.getTaillesCluster();
		int[] TaillesCumulées = ErrorRate.will.getTaillesCumulées();

		// Recupere le dictionnaire des Imagettes

		ArrayList<ArrayList<Integer>> DicoImagettes = ErrorRate.will.getImagettes();

		// On altère une des m images apprises : NumImageAlteree
		int[][] Images = ErrorRate.will.getImages();
		...
		for (int pixel=0; pixel<Scores.length; pixel++) {
			...
			// field
			for (int e:ErrorRate.Reseau.get(pixel)){
